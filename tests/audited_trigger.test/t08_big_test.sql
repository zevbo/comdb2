...
CREATE LUA AUDITED TRIGGER hello ON (TABLE t1 FOR UPDATE AND DELETE)
CREATE LUA AUDITED TRIGGER world ON (TABLE t2 FOR UPDATE AND INSERT)
...
CREATE LUA AUDITED TRIGGER world ON (TABLE t1 FOR INSERT AND UPDATE AND DELETE)
CREATE LUA AUDITED TRIGGER world2 ON (TABLE t1 FOR INSERT AND UPDATE AND DELETE)

INSERT INTO t1 VALUES(0, "doop", 4)
DROP TABLE "$audit_t1"
DROP LUA TRIGGER hello

CREATE LUA AUDITED TRIGGER hello2 ON (TABLE t1 FOR INSERT AND UPDATE AND DELETE)
