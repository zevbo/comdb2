...
int bdb_delete_single_audited_sp_tran(tran_type *tran, char *sub_table, char *audit_table){
    char **audits;
    int num_audits;
    int rc = bdb_get_audited_sp_tran(tran, sub_table, &audits, &num_audits, TABLE_TO_AUDITS);
    if (rc) {return rc;}
    ...
    for(int i = 0; i < num_audits; i++){
        ...
        if (strcmp(audit, audit_table) != 0){
            int rc = bdb_set_audited_sp_tran(tran, sub_table, audit, TABLE_TO_AUDITS);
            if (rc) {return rc;}
        }
    }
    return 0;
...
int bdb_delete_audit_table_sp_tran(tran_type *tran, char *audit, int delete_single){
    if (delete_single) {
        char **sub_table;
        int num_sub_tables;
        bdb_get_audited_sp_tran(tran, audit, &sub_table, &num_sub_tables, AUDIT_TO_TABLE);
