...
static struct schema_change_type *comdb2_alter_audit_sc(struct schema_change_type *pre, struct schema *s, char *audit){
...
void populate_alter_chain(struct dbtable *db, struct schema_change_type *sc, tran_type *tran){
    if (sc->newcsc2){
        ...
        bdb_get_audit_sp_tran(tran, sc->tablename, &audits, &num_audits, TABLE_TO_AUDITS);
        struct schema *s = create_version_schema(sc->newcsc2, -1, db->dbenv);
        for(int i = 0; i < num_audits; i++){
            add_next_to_chain(sc, comdb2_alter_audit_sc(sc, s, audits[i]));
        }
    }
}
...
int do_alter_table(struct ireq *iq, struct schema_change_type *s,
    ...
    if (s->is_monitered_alter && rc == SC_CONVERSION_FAILED && !fail_on_uncarryable_alter){
        ...
        char **triggers;
        int num_triggers;
        bdb_get_audit_sp_tran(tran, s->tablename, &triggers, &num_triggers, AUDIT_TO_TRIGGER);
